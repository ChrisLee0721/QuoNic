#!/bin/bash
# AWS Full PDB Processing Script
#
# Usage:
#   1. Launch EC2 instance (r6i.2xlarge, 100GB EBS)
#   2. Copy this script to the instance
#   3. Run: bash aws_run_full.sh
#
# Prerequisites:
#   - AWS CLI configured
#   - S3 bucket for results
#   - Python 3.10+

set -e

# Configuration
S3_BUCKET="s3://gciqa-results"
WORK_DIR="/data/gciqa"
PDB_DIR="$WORK_DIR/pdbs"
RESULTS_DIR="$WORK_DIR/results"
PYTHON_ENV="$WORK_DIR/venv"

echo "=========================================="
echo "GCIQA Full PDB Processing"
echo "=========================================="
echo "Work dir: $WORK_DIR"
echo "PDB dir: $PDB_DIR"
echo "Results dir: $RESULTS_DIR"
echo ""

# Setup
mkdir -p $WORK_DIR $PDB_DIR $RESULTS_DIR
cd $WORK_DIR

# Install dependencies
echo "[1/5] Setting up Python environment..."
if [ ! -d "$PYTHON_ENV" ]; then
    python3 -m venv $PYTHON_ENV
fi
source $PYTHON_ENV/bin/activate
pip install numpy --quiet

# Clone repo (or copy)
echo "[2/5] Getting GCIQA code..."
if [ ! -d "$WORK_DIR/PyQQQ" ]; then
    git clone https://github.com/your-repo/PyQQQ.git $WORK_DIR/PyQQQ
fi
export PYTHONPATH=$WORK_DIR/PyQQQ/src

# Download PDB files
echo "[3/5] Downloading PDB files from RCSB..."
if [ ! -f "$PDB_DIR/.download_complete" ]; then
    rsync -rlpt -v -z --delete --port=33444 \
        rsync.rcsb.org::ftp_data/structures/divided/pdb/ \
        $PDB_DIR/ \
        --parallel=4
    touch $PDB_DIR/.download_complete
    echo "Download complete: $(find $PDB_DIR -name '*.pdb.gz' | wc -l) files"
else
    echo "Already downloaded, skipping..."
fi

# Decompress
echo "[4/5] Decompressing PDB files..."
if [ ! -f "$PDB_DIR/.decompress_complete" ]; then
    cd $PDB_DIR
    find . -name '*.pdb.gz' -exec gunzip -k {} \;
    touch .decompress_complete
    echo "Decompressed: $(find . -name '*.pdb' | wc -l) files"
fi

# Run batch search
echo "[5/5] Running GCIQA batch search..."
cd $WORK_DIR
python3 -m gciqa \
    $PDB_DIR \
    --bits 4 \
    --workers 8 \
    --tolerance 0.5 \
    -o $RESULTS_DIR/full_pdb_4bit.json \
    2>&1 | tee $RESULTS_DIR/run.log

# Upload results to S3
echo "Uploading results to S3..."
aws s3 cp $RESULTS_DIR/ $S3_BUCKET/results/ --recursive

echo ""
echo "=========================================="
echo "Processing complete!"
echo "Results: $S3_BUCKET/results/"
echo "=========================================="
